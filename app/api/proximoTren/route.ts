// app/api/proximoTren/route.ts
import { NextRequest, NextResponse } from "next/server";
import axios from "axios";
import { deduplicarEstaciones, buscarEstacion, Estacion, EstacionesMap } from "@/lib/estaciones";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const estOrigen = buscarEstacion(origen, estaciones);
    const estDestino = buscarEstacion(destino, estaciones);

    if (!estOrigen) {
      return NextResponse.json({ error: `Estación origen no encontrada: ${origen}` }, { status: 404 });
    }
    if (!estDestino) {
      return NextResponse.json({ error: `Estación destino no encontrada: ${destino}` }, { status: 404 });
    }
    ...
    const tr = trResp.data.trenes.find((t: any) => t.codTren === proximo.cod_tren);
